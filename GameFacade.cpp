...
MoveOutcome GameFacade::placeLetter(int r, int c, Cell letter) {
    ...
    const Player current = tm_.current();

    if (mode_ == GameMode::Simple)
        handleSimpleMove(current, out);
    else
        handleGeneralMove(current, out);
...
void GameFacade::handleSimpleMove(const Player current, MoveOutcome& out) {
...
// GENERAL mode
void GameFacade::handleGeneralMove(const Player current, MoveOutcome& out) {
    if (out.newSequences > 0) {
        // add points for # of SOS found
        if (current == Player::Blue)
            scoreBlue_ += out.newSequences;
        else
            scoreRed_ += out.newSequences;
