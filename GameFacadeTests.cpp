...
namespace SOSCoreNativeTests {
	...
	TEST_CLASS(GameFacadeTests) {
		...
		TEST_METHOD(OwnerAtReflectsPlacer) {
			GameFacade g; g.newGame(General3());
			Assert::IsTrue(g.placeLetter(0, 0, Cell::S).placed); // Blue places
			auto own = g.ownerAt(0, 0);
			Assert::IsTrue(own.has_value());
			Assert::IsTrue(*own == Player::Blue);
		}
	};
}
